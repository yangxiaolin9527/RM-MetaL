...
class DTrainer:
    ...
    def load_data(self):
        ...
        if self.dataset == "CIFAR-FS":
            train_dataset = l2l.vision.datasets.CIFARFS(root='./dataset',
                                                        mode='train',
                                                        download=True,
                                                        transform=transform_train)
            valid_dataset = l2l.vision.datasets.CIFARFS(root='./dataset',
                                                        mode='validation',
                                                        download=True,
                                                        transform=transform_validation)
            test_dataset = l2l.vision.datasets.CIFARFS(root='./dataset',
                                                       mode='test',
                                                       download=True,
...
class ANILTrainer(DTrainer):
    ...
    def model_setup(self):
        ...
        if self.dataset == 'CIFAR-FS':
            ...
            self.inner_steps = 10
            self.fast_lr = 0.1
            self.meta_head_lr = 0.0001
            self.meta_feature_lr = 0.0001
        else:
            raise ValueError(f'{self.dataset} is not supported')
        self.feature = l2l.vision.models.ConvBase(output_size=self.ways, channels=3, max_pool=True)
        self.feature = torch.nn.Sequential(self.feature, Lambda(lambda x: x.view(-1, self.head_dim)))

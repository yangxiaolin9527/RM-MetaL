...
class DTrainer:
    ...
    def load_data(self):
        ...
        if self.dataset == "tieredImageNet":
            train_dataset = l2l.vision.datasets.TieredImagenet(root='./dataset',
                                                               mode='train',
                                                               download=False,
                                                               transform=transform_train)
            valid_dataset = l2l.vision.datasets.TieredImagenet(root='./dataset',
                                                               mode='validation',
                                                               download=False,
                                                               transform=transform_validation)
            test_dataset = l2l.vision.datasets.TieredImagenet(root='./dataset',
                                                              mode='test',
                                                              download=False,
...
class Algorithm1Trainer(DTrainer):
    ...
    def re_initialize_y(self, head, previous_head_state):
        with torch.no_grad():
            # for p, initial_p in zip(head.parameters(), previous_head_state):
            #     p.copy_(initial_p)
